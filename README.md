...
- 出图评分骨架：通过 `工具/new_output_evaluation.py` 从 Prompt Pack 和图片路径生成结构化评分 JSON，减少手写漏字段、图片路径填成非图片、failure_id 写错或重复。
...
- 结构化出图评分：`评估/output_evaluations.example.json` 和 `评估/output_evaluations.schema.json` 可记录每张图的评分、问题、失败类型 ID 和下一步动作，并限制 `image_file` 为 jpg/jpeg/png/webp 图片，禁止同一记录重复统计同一个失败类型。
