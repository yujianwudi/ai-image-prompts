...
模板已按 2026-06-10 联网复核的 OpenAI `gpt-image-2` 图片生成资料优化：更偏自然语言分段描述，少堆关键词，先明确输出用途，再写主体锁定、场景、构图、光线、材质、文字策略和安全防串约束。
...
统一质量门禁会检查 Prompt Pack 配置、角色防串审计报告、Prompt 文本质量审计、结构化出图评分记录与汇总、预览图 manifest 尺寸元数据、目录结构、本地链接、README 预览图引用、角色安全约束、参考仓库追踪、自动导出文件和单元测试。GitHub Actions 也会在 push / pull request 时自动运行同一个入口。

## 当前重点

- 角色设定防串：芙宁娜、茜特菈莉、朵莉亚/多莉。
- 固定模板拆分：场景、镜头、灯光、材质、动作、负面词、平台参数。
- gpt-image-2 优化：已联网复核 OpenAI 图片生成资料，把关键词模板改成自然语言分段提示词，并补充输出用途、文字策略、图像编辑保留项和参数建议。
...
- 全量 JSON Bundle：`生成提示词/prompt_packs.generated.json` 自动导出全部 Prompt Pack，并由 `生成提示词/prompt_packs.generated.schema.json` 描述结构；bundle 内含 `source_config_sha256`，方便前端或自动化工具核对来源配置。
- CSV 索引：`生成提示词/prompt_packs.index.csv` 自动列出 Prompt Pack、角色、模板和文件名，方便表格筛选。
...
- 结构化出图评分：`评估/output_evaluations.example.json` 和 `评估/output_evaluations.schema.json` 可记录每张图的评分、问题和下一步动作。
- 出图评分汇总：`评估/出图评分汇总.md` 自动汇总平均分、决策分布、常见问题和记录明细。
