...
用途：检查仓库结构、AGENTS 维护指引、Markdown 本地链接、README 预览图 alt/caption、预览图 manifest 尺寸/方向、角色安全约束和参考仓库追踪。
...
统一质量门禁会覆盖 Prompt Pack 配置、模板 `api_profile`、API 请求 JSONL、标签 taxonomy、角色防串审计、Prompt 文本质量审计、失败修正词库、结构化出图评分与汇总、评分骨架生成、失败修正建议、项目仪表盘、gpt-image-2 参数档位、仓库结构、Markdown 代码块闭合、预览图尺寸、README 预览图 alt/caption、安全约束、文本文件 LF / BOM / 末尾换行、自动导出文件、Python 源码编译和单元测试。单元测试本身会覆盖 Prompt Pack 渲染、api_profile 导出、API JSON payload、tags 导出、标签索引、标签 taxonomy、批量导出、CLI、预览图 manifest 同步、README 预览图 alt/caption、角色防串审计、Prompt 文本质量审计、失败修正词库、结构化出图评分与汇总、评分骨架生成、失败修正建议、项目仪表盘、gpt-image-2 参数档位和统一质量门禁帮助入口。
